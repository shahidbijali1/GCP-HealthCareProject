...
CREATE TABLE IF NOT EXISTS `gcp-de-477616.gold_dataset.financial_metrics` AS
SELECT 
...
FROM `gcp-de-477616.silver_dataset.transactions` t
LEFT JOIN `gcp-de-477616.silver_dataset.claims` c 
...
CREATE TABLE IF NOT EXISTS `gcp-de-477616.gold_dataset.payor_performance` AS
SELECT 
    c.PayorID,
    c.PayorType,
    COUNT(DISTINCT c.Claim_Key) AS TotalClaims,
...
FROM `gcp-de-477616.silver_dataset.claims` c
